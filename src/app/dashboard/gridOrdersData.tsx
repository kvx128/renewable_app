...
import { database, db } from '../firebase'; // Import your Firebase initialization
...
export const columns: GridColDef[] = [
  { field: 'Power_station', headerName: 'Power Station', flex: 1.5, minWidth: 200 },
  {
    field: 'Status',
    headerName: 'Status',
    flex: 1,
  ...
  {
    field: 'Year_operational',
    headerName: 'Year operational',
    headerAlign: 'right',
    align: 'right',
    flex: 1,
    minWidth: 80,
...
// TODO: Change the data to from the API 
...
export const fetchRows = async (): Promise<GridValidRowModel[]> => {
  const db = database;///getDatabase(app);
  const snapshot = await get(ref(db, "dashboardOrders")); // Replace with your actual node name

  if (!snapshot.exists()) return [];

  const data = snapshot.val();

  // Convert object into array of rows with 'id' field (required by DataGrid)
